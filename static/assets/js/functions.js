...
if($(window).width() < 720 ) {
   $('img').attr('src','assets/images/thumbnail/720_480' + filename);
}
if($(window).width() > 720 ) {
   $('img').attr('src','assets/images/thumbnail/1440_768' + filename);
}
});*/
...
jQuery(document).ready(function(S){
	...
	function triggerFilter($bool) {
		var elementsToTrigger = $([$('.filter-trigger'), $('.filter'), $('.tab-filter'), $('.portfolio')]);
	...
	$(".filter-content input[type='search']").keyup(function(){
	  	...
	  	delay(function(){
	    	...
	    	if ((inputText.length) > 0) {            
	      		...
	      		$('.portfolio ul').mixItUp('filter', $matching);
	    	} else {
	      		// resets the filter to show all item if input is empty
	      		$('.portfolio ul').mixItUp('filter', 'all');
	    	}
	  	}, 200 );
	});
});
...
$(document).ready(function(){
  ...
  $(".portfolio").imagesLoaded(function() {
    var $portfolioWrapper = $(".portfolio").isotope({
